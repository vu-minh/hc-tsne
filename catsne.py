...
def compute_rel_obj_diff(prev_obj, obj, n_eps):
    ...
    """
    return np.abs(prev_obj - obj) / np.maximum(
        n_eps, max(np.abs(prev_obj), np.abs(obj))
    )
...
def catsne_g(X_lds, sigma_ijt, nit, eei, eef, n_eps):
    ...
    s_ijt, log_s_ijt, idsld = catsne_ld_sim(
        ds_ld=scipy.spatial.distance.squareform(
            X=scipy.spatial.distance.pdist(X=X_lds, metric="sqeuclidean"),
            force="tomatrix",
        ),
        n_eps=n_eps,
    )
            ...
            ds_hd = (
                scipy.spatial.distance.squareform(
                    X=scipy.spatial.distance.pdist(
                        X=X_hds, metric=hd_metric, **D_hd_metric
                    ),
                    force="tomatrix",
                )
                ** 2.0
...
def viz_digits(
    X, lab, tit="", cmap="gnuplot2", stit=30, slab=15, wlab="bold", max_ti=None
):
    """
...
if __name__ == "__main__":
    ...
    for theta in Ltheta:
        ...
        d_ld = scipy.spatial.distance.squareform(
            X=scipy.spatial.distance.pdist(X=X_lds, metric="euclidean"),
            force="tomatrix",
        )
