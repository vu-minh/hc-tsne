...
def catsne_g(X_lds, sigma_ijt, nit, eei, eef, n_eps):
    ...
    s_ijt, log_s_ijt, idsld = catsne_ld_sim(
        ds_ld=scipy.spatial.distance.squareform(
            X=scipy.spatial.distance.pdist(X=X_lds, metric="sqeuclidean"), force="tomatrix"
        ),
        n_eps=n_eps,
    )
...
def catsne_mgd(
    ...
    grad, obj = catsne_g(
        X_lds=X_lds, sigma_ijt=sigma_ijt, nit=nit, eei=eei, eef=eef, n_eps=n_eps
    )
    ...
    stepsize, delta_bar = (
        epsilon * np.ones(shape=(N, dim_lds), dtype=np.float64),
    ...
    while (
        (nit <= eei)
        or (nit <= mom_t)
        or ((nit < nit_max) and (gradn > gtol) and (rel_obj_diff > ftol))
    ):
        # Computing the step sizes, following the delta-bar-delta rule, from [7].
        delta_bar, stepsize = dbd_rule(
            delta_bar=delta_bar, grad=grad, stepsize=stepsize, kappa=kappa, phi=phi, tdb=tdb
        )
        # Performing the gradient descent step with momentum.
        X_lds, up_X_lds = mgd_step(
            X=X_lds,
            up_X=up_X_lds,
            nit=nit,
            mom_t=mom_t,
...
def catsne(
    ...
    if (theta < 0.5) or (theta >= 1):
        raise ValueError(
            "Error in function catsne: theta={theta} whereas it must range in [0.5,1[.".format(
                theta=theta
            )
        )
    # Checking rand_state
    ...
    if isinstance(init, str):
        if init == "ran":
            ...
            X_lds = sklearn.decomposition.PCA(
                n_components=dim_lds, copy=True, random_state=rand_state
            ).fit_transform(X_hds)
        else:
            raise ValueError(
        ...
        if init.ndim != 2:
            raise ValueError(
                "Error in function catsne: init.ndim={v} whereas init must be a 2-D numpy array.".format(
                    v=init.ndim
                )
            )
        if init.shape[0] != N:
            raise ValueError(
                "Error in function catsne: init.shape[0]={v} whereas it must equal N={N}.".format(
                    v=init.shape[0], N=N
                )
            )
        ...
        if hd_metric == "precomputed":
            ...
            ds_hd = (
                scipy.spatial.distance.squareform(
                    X=scipy.spatial.distance.pdist(X=X_hds, metric=hd_metric, **D_hd_metric),
                    force="tomatrix",
                )
                ** 2.0
            )
...
def viz_digits(X, lab, tit="", cmap="gnuplot2", stit=30, slab=15, wlab="bold", max_ti=None):
    ...
    for i in range(N):
        ax.text(
            ...
            y=X[i, 1],
            s=str(lab[i]),
            fontsize=slab - 10.0 * (1.0 - max_ti[i]),
            color=cmap_obj(normalizer(lab[i])),
            fontdict={"weight": wlab, "size": slab - 10.0 * (1.0 - max_ti[i])},
...
def viz_qa(
    ...
    ymticks = np.linspace(
        start=ymin, stop=ymax, num=1 + mymticks * (nyMticks - 1), endpoint=True, retstep=False
    )
...
if __name__ == "__main__":
    ...
    d_hd = scipy.spatial.distance.squareform(
        X=scipy.spatial.distance.pdist(X=X_hds, metric=hd_metric), force="tomatrix"
    )
    ...
    for theta in Ltheta:
        ...
        d_ld = scipy.spatial.distance.squareform(
            X=scipy.spatial.distance.pdist(X=X_lds, metric="euclidean"), force="tomatrix"
        )
        # Displaying the LD embedding
        viz_digits(
            X=X_lds,
            lab=labels,
            tit="cat-SNE ($\\theta$={theta})".format(theta=theta),
            max_ti=max_ti,
        )
        print(
            "Computing the DR quality of the result of cat-SNE with threshold theta = {theta}".format(
                theta=theta
            )
        )
        rnxk, auc_rnx = eval_dr_quality(d_hd=d_hd, d_ld=d_ld)
        print(
            "Computing the KNN gain of the result of cat-SNE with threshold theta = {theta}".format(
                theta=theta
            )
        )
        ...
        L_kg.append(kg)
        Lleg_rnx.append(
            "{a} cat-SNE ($\\theta={theta}$)".format(
                a=int(round(auc_rnx * 1000)) / 1000.0, theta=theta
            )
        )
        Lleg_kg.append(
            "{a} cat-SNE ($\\theta={theta}$)".format(
                a=int(round(auc_kg * 1000)) / 1000.0, theta=theta
            )
        )
        Lls.append("solid")
